...
class BaseElement(object):
    ...
    def get_displayed_elements(condition, locator):
        ...
        try:
            for ele_number in range(1, element_size + 1):
                ...
                Logger.info("Finding an element with a locator " + element_locator)
    ...
    def send_keys_without_click(self, key):
        Logger.info("send_keys: Changing the text for an element '" + self.get_name() + " " + self.__class__.__name__ +
                    "'" + "' to text => '" + key + "'")
    ...
    def click(self):
        Logger.info("click: Clicking on an element '" + self.get_name() + " " + self.__class__.__name__ + "'")
    ...
    def get_text(self):
        Logger.info("get_text: Getting text for an element '" + self.get_name() + "'")
        self.wait_for_is_present()
        text = self.find_element().text
        Logger.info("get_text: Text received '" + text + "'")
        return text
    ...
    def get_attribute(self, attr):
        Logger.info("get_attribute: Getting an Attribute " + attr + " for element '" + self.get_name() + "'")
        self.wait_for_is_visible()
        return self.find_element().get_attribute(name=attr)

    def get_css_value(self, property_name):
        Logger.info("get_attribute: Getting a CSS Attribute" + property_name + " for element '" + self.get_name() + "'")
        self.wait_for_is_visible()
    ...
    def scroll_by_script(self):
        self.wait_for_is_visible()
        Logger.info("Scroll to element '" + self.get_name() + "'")
        Browser.get_browser().execute_script(scripts_js.SCROLL_INTO_VIEW, self.find_element())

    def double_click(self):
        self.wait_for_is_visible()
        Logger.info("double_click: Double click on an element '" + self.get_name() + "'")
    ...
    def wait_for_is_visible(self):
        ...
        self.wait_for_check_by_condition(method_to_check=waiter, message=" not visible")
    ...
    def wait_for_is_absent(self, msg=" exists"):
